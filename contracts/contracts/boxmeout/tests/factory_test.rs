...
use soroban_sdk::{
    ...
    token, Address, BytesN, Env, Symbol, contract, contractimpl,
};
...
fn register_mock_treasury(env: &Env) -> Address {
    env.register(MockTreasury, ())
}
...
fn test_factory_initialize() {
    ...
    let treasury = register_mock_treasury(&env);
